...
public class UKSouth implements Runnable, Serializable{
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, EOFException {
        ...
        System.out.println("Port Number connecting first UK South Server and Scheduler Server " + portNumber);
        ...
        try{
            while(true){
                ...
                if(taskMap.containsKey("dirty")){
                    ...
                    new UKSouth().dirtyEnergy(listOfTasks);
                }
                else if(taskMap.containsKey("Clean")){
                    listOfTasks = taskMap.get("Clean");
                    new UKSouth().cleanEnergy(listOfTasks);
                }
            }
    ...
    public void cleanEnergy(List<ServerClientProtocol> listOfTasks){
        for (int i = 0; i < listOfTasks.size(); i++) {
            new UKSouth(listOfTasks.get(i).getTaskID());
        }
    }
